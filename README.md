...
| 5. 内容页面 | 完成文章、资料库、时间线、关于、搜索等页面 | 下一阶段 |
...
- 已冻结并实现“首页 / 文章 / 资料库 / 旅程 / 关于”的首发主导航；详情与基础列表已建立，筛选和主题聚合将在阶段 5 完成。
...
- 首屏核心句、CTA、站点统计和抽象轨道主视觉在桌面与移动端均完整可见。
...
- 已建立文章、资料、短想法和 Journey 的集合入口页；阶段 5 将继续补齐筛选与聚合。
- 首页动效只用于轻微漂浮与滚动揭示，并遵守 `prefers-reduced-motion`。
...
- 各类型已有详情与基础入口页，但资料筛选、主题/标签聚合、相关文章和独立搜索页将在阶段 5 实现。
...
下一阶段是“内容页面”。将深化文章阅读与列表、资料筛选、短想法、Journey 时间线、About、归档、主题/标签聚合、独立搜索、404 和版权/隐私页面，贯通全部内容发现路径。
