...
| 1. 品牌与内容盘点 | 明确站点名称、个人表达、栏目和首发素材 | 待开始 |
| 2. 设计系统 | 建立颜色、字体、间距、组件和动效规范 | 待开始 |
...
- Astro 生产构建成功，共生成 29 个静态页面。
- Pagefind 构建成功，索引 22 个公开页面、2020 个词条，覆盖中文和英文。
- 构建过程完成 6 张图片优化。
- CMS 的 Vite 生产构建成功。
- 本地生产预览成功启动。
...
- 关于与友链：`src/content/spec/`
- 图片资源：`src/assets/` 或对应文章目录。
- 网站配置：`src/config.ts`
- 内容 schema：`src/content.config.ts`
- 国际化：`src/i18n/`

后续阶段会逐步迁移为 `articles`、`resources`、`thoughts`、`journey` 和 `pages` 等内容集合。在迁移完成前，不应同时维护两套真实内容。
...
- 仍显示模板作者的名称、头像、链接和示例文章。
- 评论功能已关闭；在部署自有服务并完成隐私审查前不会启用。
- 模板作者的 Umami 统计脚本已移除，当前不会向其统计服务发送访问数据。
- 尚未建立资料库、短想法和经历内容集合。
- 尚未实现 Apple 发布会式的新设计系统。
- 本地 CMS 目前主要管理博客文章，尚未支持全部目标内容类型。

下一阶段是“个人品牌与内容盘点”。在开始视觉重建前，需要确认站点名称、首页核心句、头像/主视觉、关注领域、公开经历、联系方式、语言策略和首发素材。
